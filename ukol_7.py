...
class Auto:
    def __init__(self, registracni_znacka, znacka_typ_vozidla, najete_km):
        self.registracni_znacka = registracni_znacka
        self.znacka_typ_vozidla = znacka_typ_vozidla
        self.najete_km = najete_km
        self.vozidlo_volne = True

    def __str__(self): # specialni metoda
        return f"Vozidlo {self.registracni_znacka}, {self.znacka_typ_vozidla}"


    def pujcAuto(self):
        if self.vozidlo_volne:
            self.vozidlo_volne = False
            return f"Potvrzuji zapůjčení vozidla {self.registracni_znacka}, {self.znacka_typ_vozidla}"
            
        else:
            return f"Vozidlo není k dispozici {self.registracni_znacka}, {self.znacka_typ_vozidla}"

    def getInfo(self):
        return f"Info o vozidle {self.registracni_znacka}, {self.znacka_typ_vozidla}"
    
Peugeot = Auto("4A2 3020", "Peugeot 403 Cabrio", 47534)
Skoda = Auto("1P3 4747", "Škoda Octavia", 41253)

print(f"Pujcovna pujcuje tyto auta")
print(Peugeot.getInfo())
print(Skoda.getInfo())

vyber_auta = input(f"Vyber znacku Peugeot/Skoda k zapujceni")
if vyber_auta == 'Skoda':
    print(Skoda.getInfo())
    ...
    print(f"Toto auto {vyber_auta} pujcovna nema")
